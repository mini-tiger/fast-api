...
// UploadFileStream 直接上传外部链接
func (c *ClientType) UploadFileStream(url, dir, fileName string, metadata map[string]string) (string, error) {
	// 服务端上传用内网
	c.SetUseInternalEndpoint(true)
	// 开发环境用外网
	if core.Mode == core.Dev {
	...
	// 上传
	_, err = c.GetClient().PutObject(context.Background(), putRequest)
	if err != nil {
		return "", err
	}
	// 返回新地址
...
func (c *ClientType) GetPreUploadFileUrl(dir, fileName string, metadata map[string]string) GetPreUploadFileUrlResultType {
	...
	result, err := c.GetClient().Presign(context.TODO(), &oss.PutObjectRequest{
		...
		Metadata:     metadata,                           // 请确保在服务端生成该签名URL时设置的Metadata与在使用URL时设置的Metadata一致
	},
